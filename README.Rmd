...
##### This repository houses scripts and functions used for cleaning, aggregating and manipulating, NFL play-by-play data to feed into machine learning models that attempt to predict the winners of NFL games. The data used in modeling was obtained using the `nflfastR` package. All of the in-house functions mentioned below are located in the `utils.R` script in the `utils/` directory of this repo.
...
```{r, fig.align='center'}
  ...
  ggplot2::ggplot() +
  ggplot2::geom_line(aes(x = season, y = value, col = name)) +
