...
Below is the variable importance scores for the Logistic Regression and the XGBoost models. The most important variables in both models are both teams Elo ratings, the cumaltive average score differential and the opponent's winning percentage.  
...
# log_reg_vip <-
  ...
  # ggplot2::labs( title = "Variable Importance", subtitle = "Logisitc Regression",y = "Variable") +
  # apatheme
# log_reg_vip + xgb_vip
```
...
vip_df %>% 
  ...
  ggplot2::ggplot() +
  ggplot2::geom_col(aes(x = Importance, y = Variable)) +
...
# Test Metrics
I then selected the tuning parameters for each model that had the best performance metrics on the data resamples. Using these model parameters, I refit each model using the initial data split. I then used the final fitted model to predict on the training and test sets. Below I've summarised the models performance on the training and testing data splits. 
...
metrics_df %>% 
  ...
  dplyr::rename(mean_log_loss = mn_log_loss, "Data split" = data, sensitivity = sens, specificity = spec) %>% 
  flextable::flextable() %>% 
  flextable::add_header_row(
...
metrics_df %>% 
  ...
  dplyr::mutate(across(where(is.numeric), round, 2)) %>% 
  ggplot2::ggplot() +
  ggplot2::geom_line(aes(y = model, x = .estimate)) + 
  ggplot2::geom_point(aes(y = model, x = .estimate, col = data)) +
  ggplot2::facet_wrap(~.metric) +
  apatheme + 
  labs(
    title = "Performance of the best models"
  )
...
## ROC Curves
Receiver Operator Characteristic curves, (aka ROC curves) are a way of understanding how well a model is predicting the True Positive events vs. the False Positive events. In this scenario a True Positive event is the model predicts a win and the team wins, while a False Positive event is the model predicts a win, and the team loses. The True Positive Rate (TPR) is on the Y axis and the False Positive Rate (FPR) is on the X axis. The TPR, or sensitivity, along the Y axis indicates the probability that the model predicted a win and the team actually won. While FPR or, 1 - specificity, along the X axis indicates the probability that the model predicted a win and the team actually lost. 
