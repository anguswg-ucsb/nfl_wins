...
Another note is that I ended up only using data from the perspective of the home team. 
Initially, I had been running models with 2 data points per game, one being the home team perspective and the other being the away team perspective. I quickly ran into some major overfitting in the models, and realized it was a better idea to only use data from the home team.
...
data, so data that was collected *after* the game (i.e. data not available to someone before the game starts, like the number of turnovers in the game). However, if I wanted to make any useful predictions about the upcoming week, I would also need to create ***a priori*** data (i.e. data available *before* the event we want to predict).
...
-   All of the models made more correct predictions than Las Vegas, doing a better job correctly predicting losses than Las Vegas

-   All of the models have higher specificity than Las Vegas, they all made more correct prediction than Las Vegas
...
-   In general, the models have the most trouble classifying False Positive cases
...
Let’s look at the 25% of data—1373 games—that we set aside as a testing dataset. 
We can now generate predictions for those games, and calculate the percent of games per year where our models correctly predicted the outcome (shown in the graph below).
...
Looks like our predictions are right in line with Vegas! Note that some weeks we do a little better than Vegas, some weeks a little worse.
