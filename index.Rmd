...
# **REST API and nflpredictr**
To access predictions from the models discussed in this document, I have built out a [REST API](http://68.183.25.9:8000/__docs__/) that scrapes current game data, feeds this data into the above Logistic Regression model, and returns predictions to end users. 

I've also made an R package called [`nflpredictr`](https://github.com/anguswg-ucsb/nflpredictr) to accompany the REST API and to simplify the process of making requests to the API. The package can be installed from GitHub like so:
...
Another thing to note is that all the data you see is from the perspective of the home team, and all model predictions indicate a home win or home loss.
