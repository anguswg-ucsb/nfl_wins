...
# fantasy             <- readRDS(here::here("data", "fp_team_rollmean_data.rds"))
fantasy             <- readRDS(here::here("data", "fp_team_cuml_data.rds"))
...
fp_df <-
  ...
  # dplyr::select(-avg_fp_rank) %>% 
  dplyr::select(season, week, team, fp_hppr, fp_rank, home, contains("avg")) %>% 
  # dplyr::select(season, week, team, position, fp_hppr:receiving_ypt) %>%
  # dplyr::select(home, season, week, team, position,fp_hppr, touches:rushing_epa, receptions:catch_rate, 
  #               receiving_yards:receiving_epa, win_pct_rmean) %>%
  dplyr::group_by(season, week) %>% 
  dplyr::mutate(
    # fp_rank = rank(-fp_hppr, ties.method = "min"),
    fp_finish = dplyr::case_when(
      fp_rank <= 5 ~ 1,
      fp_rank > 5  ~ 0
    ),
  ...
  dplyr::ungroup() %>% 
  dplyr::select( -fp_hppr, -season, -fp_rank, -avg_dakota, -avg_passing_2pt_conversions) %>% 
  dplyr::relocate(fp_finish)
  # dplyr::group_by(team) %>%
  # dplyr::arrange(season, week, .by_group = T) %>%
  # tk_augment_lags(c(touches:rushing_epa, receptions:catch_rate, 
  #                   receiving_yards:receiving_epa), .lags = 1:4) %>%
  # tk_augment_slidify(
  #   contains("lag4"),
  #   .f       = ~mean(.x, na.rm = T),
  #   .period  = 4,
  ...
  # ) %>% 
  # dplyr::ungroup() %>% 
  # dplyr::select(season, fp_hppr,team, home, contains("lag")) %>% 
  # na.omit() %>% 
  # dplyr::mutate(
  #   # season = factor(season)
  #   season_range = dplyr::case_when(
  #     season >= 1999 & season < 2005  ~ "range1",
  #     season >= 2005 & season < 2011  ~ "range2",
  #     season >= 2011 & season < 2017  ~ "range3",
  #     season >= 2017                  ~ "range4"
  #   ),
  #   season_range = factor(season_range)
  # ) %>% 
  # dplyr::select(-season) %>% 
  # dplyr::relocate(season = season_range) 
...
fp_df  <- 
  ...
  # dplyr::select(season, week, team, home, fp_hppr, contains("avg_")) %>%
  dplyr::select(season, week, team, home, fp_hppr, contains("avg_")) %>%
  # dplyr::select(-contains("avg_")) %>%
  # dplyr::select(-avg_dakota, -avg_passing_2pt_conversions)  %>% 
  dplyr::mutate(
    # season = factor(season)
    season_range = dplyr::case_when(
      season >= 1999 & season < 2005  ~ "range1",
  ...
  # dplyr::select(team, home, fp_hppr)
  # dplyr::select(team, home, fp_hppr, 29:67) %>%
  # na.omit()
fp_corr <-
  fp_df %>% 
...
#   dplyr::select(team, home, fp_hppr, contains("roll_8")) %>%
#   setNames(c(gsub("_lag1|_roll_8", "", names(.)))) %>%
#   dplyr::select(
#     fp_hppr,
#     # fp_finish,
#     team, # player_id,
#     home,  win_pct_rmean,# names(.)[12:53], names(.)[56:97]
#     fp_rank,
...
fp_split <- initial_split(fp_df, strata = week)
...
hist(fp_train$fp_hppr, breaks = 30)
hist(fp_test$fp_hppr, breaks = 30)

sum(fp_train$fp_hppr)
sum(fp_test$fp_hppr)
...
usemodels::use_kknn(fp_finish~., data = fp_train)
usemodels::use_xgboost(fp_finish~., data = fp_train)
usemodels::use_glmnet(fp_finish~., data = fp_train)
usemodels::use_earth(fp_finish~., data = fp_train)
...
# CV folds
fp_folds <- rsample::vfold_cv(fp_train, v = 10, strata = week)
